...
Este repositório parte do setup público acima e já vem com um time de 4 subagentes (fixo)
...
O time vem com: `paulo-dev`, `juliana-ops`, `jonathan-copy`, `rafael-projetos`.
...
**Este fork** entrega, além da base pública acima:
- 4 subagentes especializados (ver lista completa na seção "Sobre este fork" acima)
- Audio bidirecional (faster-whisper local na entrada + ElevenLabs/Gemini na saida)
- Memoria vetorial PostgreSQL + pgvector (embeddings 384-dim, sem OpenAI)
- Sem agent-manager.py/PM2/Caddy/Cloudflare tunnel nem "Clone SDR" - isso era de uma versao
  generica anterior e nao faz parte do que este repo entrega
...
[Telegram]  <==>  [Bot Python systemd 24/7]  <==>  tmux send-keys + outbox JSON
                   ...
                   [audio Whisper / TTS]          [Claude Code (tmux)]
                                  ...
                                  +---------+---------+---+---+
                                  v         v         v       v
                              paulo-dev juliana-ops jonathan rafael   (fixo, sem 5o agente)
```

Camadas de resiliencia:
- Bot Python systemd `Restart=always`
- Claude Code tmux + `start-agente.sh` (mata e recria a sessao se precisar)
- Healthcheck a cada 2 min via cron, com auto-alerta e auto-restart
- Restart diario do agente (crontab-referencia.txt) pra limpar contexto acumulado

## Recursos deste fork

- Bot externo Python sempre vivo (independente do Claude Code)
- Audio entrada (faster-whisper local PT-BR) e saida (ElevenLabs/Gemini TTS) bidirecional
- "Digitando..." automatico durante todo processamento
- 4 subagentes especializados (ver lista na seção "Sobre este fork")
- Memoria vetorial PostgreSQL + pgvector (HNSW index, embeddings 384-dim locais)
- Healthcheck a cada 2 min com auto-alerta e auto-restart
- Sistema de promessas/lembretes e fila de aprendizado (ver `CLAUDE.md`)
