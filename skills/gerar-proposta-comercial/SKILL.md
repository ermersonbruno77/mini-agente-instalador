...
> **Importante:** o subdominio final e `<NOME>.<DOMINIO_BASE>`, onde `DOMINIO_BASE` e definido pelo proprio aluno no `.env` do agente (ex: `meunegocio.com.br`, `agencia.com`). NAO e mais hardcoded para `exemplo-antigo.com`. Se o aluno nao configurar `DOMINIO_BASE`, o agente avisa e mostra como configurar.
...
O aluno precisa configurar em `/opt/{{AGENTE_NAME_LOWERCASE}}/.env` (ou no `.env` do projeto) antes de rodar:
