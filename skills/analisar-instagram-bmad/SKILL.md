...
### Etapa 5: Deploy
1. Le `DOMINIO_BASE` do .env do agente (`/opt/{{AGENTE_NAME_LOWERCASE}}/.env` ou env exportada). Se nao existir, aborta e instrui o aluno: "Falta configurar DOMINIO_BASE no /opt/{{AGENTE_NAME_LOWERCASE}}/.env. Te explico como." e mostra o passo a passo.
...
O aluno cadastra no `/opt/{{AGENTE_NAME_LOWERCASE}}/.env` do agente dele (NAO no codigo da skill). A skill apenas le essas variaveis em runtime:
...
> "Falta configurar `DOMINIO_BASE` no /opt/{{AGENTE_NAME_LOWERCASE}}/.env. Te explico como."
