...
Sinais de saude:
- Metodo proprio com nome (ex: "Metodo X de Vendas")
...
A skill NAO usa o dominio do {{DONO}}. Cada aluno usa o proprio dominio. O subdominio gerado e sempre `<USERNAME_DO_INSTAGRAM>.<DOMINIO_BASE>`, onde `DOMINIO_BASE` vem do `.env` do agente do aluno.
...
### Variaveis que o aluno cadastra no /opt/{{AGENTE_NAME_LOWERCASE}}/.env
