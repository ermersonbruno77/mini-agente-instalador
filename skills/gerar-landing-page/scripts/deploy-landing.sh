...
#
# REQUISITOS:
#  - .env carregado (em /opt/{{AGENTE_NAME_LOWERCASE}}/.env OU ./.env)
...
# ---------------------------------------------------------------
ENV_LOADED=0
for ENV_FILE in "/opt/{{AGENTE_NAME_LOWERCASE}}/.env" "./.env" "$(dirname "$0")/../../.env"; do
  if [[ -f "$ENV_FILE" ]]; then
    set -a
...
1. Edite /opt/{{AGENTE_NAME_LOWERCASE}}/.env (ou o .env do seu projeto) e adicione:
